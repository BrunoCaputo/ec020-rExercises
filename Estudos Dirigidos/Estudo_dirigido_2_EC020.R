...
plot(airquality$Wind, airquality$Temp, xlab = "Vento", ylab = "Temperatura",
     main = "Vento x Temperatura", col = "red", type = "h")
...
nrow(no_na_airquality)
ncol(no_na_airquality)

# 7) Faça um histograma, "também elegante", a respeito da coluna Solar.R, com 
# no mínimo 5 parametros descritos acima (col, main....)
hist(airquality$Solar.R, col = "blue", main = "Histograma Solar.R", 
     ylab = "Frequência", xlab = "Valor", breaks = c(0,100,300,500), ylim = c(0,0.004))
...
covid <- matrix(c(600, 400, 50, 406, 78, 554, 100, 54, 45, 47, 120, 134),
                nrow = 4, byrow = T)

# Dados fictícios
colnames(covid) <- c("Suspeitos", "Confirmados", "Descartados")
rownames(covid) <- c("Espanha", "Alemanha", "Argentina", "Bolívia")

barplot(covid, col = c("red", "green", "blue", "purple"),
        xlab = "Casos", ylab = "Países", beside = T)
...
fac_species <- factor(iris$Species)
sum_fac_species <- summary(fac_species)
lbls <- c("Setosa", "Versicilor", "Virginica")
species_colors <- c("blue", "red", "yellow")
pie(sum_fac_species, labels = lbls, col = species_colors)
